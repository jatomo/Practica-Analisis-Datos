...
```{r}
datos_hospital_relevantes <-datos_hospital_Practica  |> 
  remove_empty(c( "cols")) |> 
  select (-merged_header, -...20, -fiebre)
...
```{r}
datos_hospital_cambio <-datos_hospital_relevantes|> 
  mutate(across(genero, function(x) ifelse(x == 'f', 'F', ifelse(x == 'm', 'M', x)))) |> 
  mutate(across(genero, toupper))
```
...
```{r}
datos_hospital_fecha <- datos_hospital_cambio |> 
  mutate(fecha_del_resultado = dmy(fecha_del_resultado))

```

```{r}
glimpse(datos_hospital_fecha)
glimpse(datos_hospital_cambio)
```
